...
public class DAO {
    ...
    public void selectAll() throws SQLException {
        String strX = "SELECT * FROM " + tableName;
        logger.debug("Retrieving all DB entries in < " + tableName + " > table.");
        ResultSet resultSet = connection.createStatement().executeQuery(strX);
    ...
    public void strY() throws SQLException {
        String strY = "DELETE FROM " + tableName;
        logger.warn("Deleting all entries in DB table < " + tableName + " >.");
        connection.createStatement().executeUpdate(strY);
    }
}
